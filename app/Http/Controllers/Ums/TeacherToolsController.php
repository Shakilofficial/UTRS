...
class TeacherToolsController extends Controller
{
    ...
    {
        $my_req    = Requisition::where('user_id', Auth::id())->count();
        $pending   = Requisition::where(['status' => 'Pending'])->where('user_id', Auth::id())->count();
        
        $approved  = Requisition::where('user_id', Auth::id())->where(function($query) {
                                $query->where('status', 'Approved By Administration')
                                      ->orWhere('status', 'Approved By Vice Chancellor');
                     })->count();

        $declined  = Requisition::where('user_id', Auth::id())->where(function($query) {
                                $query->where('status', 'Declined By Administration')
                                      ->orWhere('status', 'Declined By Vice Chancellor');
                     })->count();

    	return view('backend.teacher.index', compact('my_req', 'pending', 'approved', 'declined'));
    }
    ...
    public function cancel_requisition(Request $request)
    {
        $id = $request->id;
        ...
        Session::flash('error', 'Requisition Cancelled Successfully !');
